...
int substitute(int input) {
    int output = 0;
    for (int i = 0; i < 8; i++) {
        int six_bit_block = (input >> (6 * (7 - i))) & 0b111111;
        int row = ((six_bit_block & 0b100000) >> 4) | (six_bit_block & 0b000001);
        int col = (six_bit_block >> 1) & 0b1111;
        output |= (S_box[i][row][col] << (4 * (7 - i)));
    }
    return output;
}
...
int Encode_64_bit_Data(string s){
    // x should call the function that converts the string to to hex not like hexa key if the input is not hex
    int x = convert_hexaKey_into_hex(s);
